...
const ShowDown = {
  ...
  methods: {
    ...
    showScoreboard(winner) {
      let scoreboardText = this.losers
        .map(
          (loser) =>
      ...
      this.scoreboardHTML = scoreboardText;
    },
    nowTime() {
    ...
    handleLosers(e) {
      if (this.losers.map((loser) => loser.key).includes(e.key)) return;
      let delta = this.bangTime - nowTime();
      this.losers.push({ key: e.key, delta: delta });
    },
    ...
    waitForWinnerEvadingLosers() {
      document.addEventListener(
        "keydown",
        (e) => {
          if (this.losers.map((loser) => loser.key).includes(e.key)) {
            waitForWinnerEvadingLosers();
            return;
          }
    ...
    randomBetween(lowBound, highBound) {
      return Math.random() * (highBound - lowBound) + lowBound;
    },

    showdown(fromTime, toTime) {
